# The code originated by DeepMind Technologies has been slightly
# modified.  Main modification is replacing batch norm with group norm (groups = 2).
# Modifications were made by Andrew Kan andrew.k.kan@gmail.com 
#
...
class BlockV1(hk.Module):
  ...
  def __init__(
    ...
    if bottleneck:
      ...
      gn_2 = hk.GroupNorm(name="groupnorm_2", scale_init=jnp.zeros, **gn_config)
...
class ResNet(hk.Module):
  ...
  def __init__(
    ...
    Args:
      ...
      gn_config: A dictionary of two elements, ``groups`` and ``eps`` to be
        passed on to the :class:`~haiku.groupnorm` layers. By default the
        ``groups`` is ``2`` and ``eps`` is ``1e-5``.
    ...
    if not self.resnet_v2:
      self.initial_groupnorm = hk.GroupNorm(name="initial_groupnorm",
                                            **gn_config)
  ...
  def __call__(self, inputs=False):
    out = inputs
...
class ResNet18(ResNet):
  ...
  def __init__(
    ...
    Args:
      num_classes: The number of classes to classify the inputs into.
      gn_config: A dictionary of two elements, ``groups`` and ``eps`` to be
        passed on to the :class:`~haiku.groupnorm` layers.
